...
class EspolCasStrategy {
  ...
  async validateTicket(ticket, req) {
    ...
    try {
      ...
      if (!success) {
        throw new Error('No se encontró cas:authenticationSuccess en la respuesta');
      }
      ...
      const attributes = {};
      const attributesElement = success.getElementsByTagName('cas:attributes')[0];
      if (attributesElement) {
        for (let i = 0; i < attributesElement.childNodes.length; i++) {
          const node = attributesElement.childNodes[i];
          if (node.nodeType === 1) { // Element node
            const attrName = node.nodeName.replace('cas:', '');
            attributes[attrName] = node.textContent.trim();
          }
        }
      }
      ...
      const user = {
        id: username,
        username: username,
        email: attributes.mail || attributes.email || `${username}@espol.edu.ec`,
        displayName: attributes.displayName || attributes.cn || attributes.nombre || username,
